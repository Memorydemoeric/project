...
def purchase_detail_input(pro_id, ord_id, pro_count):
    pro_infos = StorageProduct.objects.filter(pro_id=pro_id)
    pur_infos = Purchase.objects.filter(pk=ord_id)
    ...
    if pro_infos and pur_infos:
        ...
        if repeat_pro:
            ...
            pur_detail.pur_pro_id_id = pro_id
            pur_detail.pur_half_finished_id_id = pro_id
